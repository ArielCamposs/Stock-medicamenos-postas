...
function primerErrorConsulta(
  ...partes: ({ error?: { message: string } | null } | null)[]
): string | null {
...
export default async function PostaDescuentoPage({ params, searchParams }: PageProps) {
  ...
  try {
    ...
    const errorConsulta = primerErrorConsulta(
      medicamentosRes,
      curStockRes,
      movsRes,
      avisRowsRes
    ...
    const msg =
      e instanceof Error
        ? e.message
        : "No se pudo cargar el descuento diario de este mes.";
